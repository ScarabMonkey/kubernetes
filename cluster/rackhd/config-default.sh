...
# need SKU
if [ -z "$RACKHD_SKU" ]; then
	echo "Need to set RACKHD_SKU"
	exit 1
fi

function getnode {
  local sku=$1
  local rack_id=$(rackhdcli nodes list -q --sku=${sku} --without-tags=k8s | head -n 1)
  echo $rack_id
}

function getip {
  local node=$1
  local ip=$(rackhdcli nodes lookup --node=${node} -q | grep -i ipaddress | awk '{print $2}')
  echo $ip
}

set +u
if [ -z "$MASTER" ]; then
	RACK_ID=$(getnode "${RACKHD_SKU}")
	NODE_IP=$(getip "${RACK_ID}")
	MASTER="$DEFAULT_USER@${NODE_IP}"
fi
set -u
echo "Master node set to ${MASTER}"
